	...
	public void run(){
		try{
			...
			while((line = br.readLine()) != null){
				//如果读到的以CrazyitProtocol.USER_ROUND开始，并以其结束则可以确定读到的是用户登录的用户名
				...
				//如果读到的行以CrazyitProtocol.PRIVATE_ROUND开始，并以其结束则可以确定是私聊信息，私聊信息只向特定的输出流发送
		...
		//捕获到异常后，表名该Socket对应的客户端已经出现了问题，，所以程序将其对应的输出流从Map中删除
		catch(IOException e){
